...
# set this variable to TRUE if GADM regions should be assigned to mines with coordinates
...
if (add_GADM == TRUE){
  ...
  general_output <- general_output %>% 
    left_join(general_output_gadm, by = "mine_id")
...
mines_without_poly_sf <- general_output %>% 
  filter(GID_1 %>% is.na()) %>% 
  select(mine_id, mine_fac, alphanumiso, geometry, GID_1, GID_2, GID_3, GID_4) %>% 
...
if (file.exists("./01_input/03_log_files/01_detailed_data/log_location_not_intersecting_with_GADM.csv")){
  ...
  if (nrow(missing_rows) > 0) {
    ...
    GADM_log <- rbind(WU_GADM_concordance, missing_rows)
    
    write_csv(WU_GADM_concordance, "./01_input/03_log_files/01_detailed_data/log_location_not_intersecting_with_GADM.csv", 
                show_col_types = FALSE)
...
general_output <- general_output %>% 
  left_join(GADM_log %>% 
              select(mine_id, GID_1, GID_2, GID_3, GID_4), 
            by = c("mine_id")) %>% 
