...
if (file.exists("./01_input/02_lists_and_concordance_tables/facility_ids.csv")){
  ...
  new_facilities <- general %>% 
     left_join(facility_ids, by = c("mine_fac", "sub_site")) %>% 
     filter(facility_id %>% is.na())
  
  #Case when new facilities need a new facility_id
  if(nrow(new_facilities) > 0) {
    
    # assign already existing facility_ids to corresponding facilities
    general_w_existing_fac_id <- general %>%
      arrange(mine_fac) %>%
      left_join(facility_ids, by = c("mine_fac", "sub_site")) %>% 
    ...
    # assign new facility_ids to new facilities without facility_id
    last_nr <- facility_ids$facility_id %>% substr(., 4, 8) %>% as.integer() %>% max()
    
    general_wo_existing_fac_id <- general %>% 
      arrange(mine_fac) %>%
      left_join(facility_ids, by = c("mine_fac", "sub_site")) %>% 
      relocate(facility_id, .before = mine_fac) %>%
      filter(facility_id %>% is.na()) %>%
      arrange(mine_fac) %>%
      group_by(mine_fac) %>% 
        mutate(facility_id = str_c("WUD", str_pad(dplyr::cur_group_id() + last_nr, width = 5, pad = "0"))) %>%
      group_by(facility_id) %>% 
        mutate(facility_id = str_c(facility_id, ".", str_pad(row_number()-1, width = 2, pad = "0", "left"))) %>% 
      relocate(facility_id, .before = mine_fac)
    
    
    # add the new facility_ids to the facility_ids.csv file 
    facility_ids <- facility_ids %>% bind_rows(general_wo_existing_fac_id %>%
                                                 select(facility_id, mine_fac, sub_site))
    
    write_csv(facility_ids, "./01_input/02_lists_and_concordance_tables/facility_ids.csv", na = "")
    
    
    # create table general which now has all facility ids assigned
    general <- bind_rows(general_w_existing_fac_id, general_wo_existing_fac_id)
    
  
  # check that general has the same number of rows as before the assignment of facility_ids
  if(nrow(general) != nrow(general_before)){
    ...
    stop(cat("The assignment of facility_ids caused the table general (nrow: ", nrow(general), ") to have more or less rows than it should have (nrow:", nrow(general_before), ")"))
  } 
    
    
  # Case when all facilities can be supplied with a facility_id from the file facility_ids.csv  
  } else {
  ...
  general <- general %>% 
    arrange(mine_fac) %>% 
    group_by(mine_fac) %>% 
      mutate(facility_id = str_c("WUD", str_pad(dplyr::cur_group_id(), width = 5, pad = "0"))) %>%
    group_by(facility_id) %>% 
      mutate(facility_id = str_c(facility_id, ".", str_pad(row_number()-1, width = 2, pad = "0", "left"))) %>% 
    relocate(facility_id, .before = mine_fac)
...
general_output <- general_output %>% 
  left_join(GADM_log %>% 
              select(facility_id, GID_1, GID_2, GID_3, GID_4), 
            by = c("facility_id")) %>% 
...
# # check 1
# general_output2 %>%
#   filter(facility_id == "WUD00114.00") %>%
