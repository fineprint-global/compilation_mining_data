...
nrow(cov_by_country %>% filter(share > 1.000000001)) / nrow(cov_by_country) 
...
## Global coverage EXCL CHINA (area chart tiles)
```{r , echo = FALSE, message = FALSE, warning = FALSE} 
## build tile chart with 9 coverage figures
area_plot_ids <- c("F.coal", "O.Fe", "O.Al", 
                   "Me.Au", "Me.Cu", "Me.Mo",  
...
dat_text2 <- tibble(label = c("incl. China", rep("", length(area_plot_ids)-1)),
                   material_name = factor(area_plot_names, levels = area_plot_names))
