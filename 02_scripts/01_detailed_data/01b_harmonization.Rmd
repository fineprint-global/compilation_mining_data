...
no_match_mat <- source_materials %>% filter(!(material %in% (source_mat_ids %>% filter(source_cat == "detailed") %>% pull(source_material_id))))
...
# table for Rmd output
datatable(no_match_unit, rownames = FALSE, caption = "Missing source_unit entries")
  ...
  sheet <- sheet %>%
    left_join(.,
              source_mat_ids %>%
                ...
                select(source_material_id, material_id), by=setNames(nm=variable_name,
                                                                  "source_material_id")
              ) %>%
